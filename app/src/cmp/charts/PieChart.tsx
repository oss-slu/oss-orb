...
export default function PieChartDiv({ title, endpoint }: pieChartProps) {
    ...
    if (error) return <div>Failed to load {title}: {error}</div>;
    if (!data) return <div>Loading {title}...</div>;

    return (
        <div style={{width: '75vw', margin: '0 auto', textAlign: 'center', border: '2px solid black'}}>
            <h2>{title}</h2>
            <ResponsiveContainer width='100%' height={400}>
                <PieChart title="test">
                    ...
                    ></Pie>
                    <Legend />
                    <Tooltip formatter={(value, name) => [`${((typeof value === 'number'
                        ? value : Number(value ?? 0)) * 100).toFixed(2)}%`, String(name)]}
                    />
                </PieChart>
            </ResponsiveContainer>
        </div>
